...
class OSRMPlanner:
    ...
    def time_moving_between_2_points(self, origin: Tuple[float, float],
...
class AntColonyOptimizer:
    def __init__(self, points: List[Tuple[float, float]],
                 ...
                 n_ants: int = 10,
                 n_iterations: int = 50,
                 decay: float = 0.5, #коэффициент испарения феромона
                 alpha: float = 1, #вес феромона при выборе пути
                 beta: float = 2): # вес эвристики при выборе пути
        self.points = points
    ...
    def _compute_time_matrix(self):
        for i in range(self.n_points):
            for j in range(self.n_points):
                if i != j:
                    self.time_matrix[i][j] = self.planner.time_moving_between_2_points(
                        self.points[i], self.points[j], self.mode)
...
if __name__ == "__main__":
    points = [
        ...
        (55.800426, 49.111931),  # Дворец земледельцев
        (55.790939, 49.114654),  # Мечеть Кул Шариф
        (55.796283, 49.106993),  # Памятник коту казанскому
    ]
    priorities = [1, 1, 3, 4]

    while True:
        mode = input("Выберите тип передвижения (walking/driving): ").lower()
        if mode in ['walking', 'driving']:
            break
        print("Некорректный ввод. Пожалуйста, введите 'walking' или 'driving'")
    ...
    route_map = visualize_route(points, best_path, planner, mode)  # Используем выбранный режим
    if route_map:
        filename = f"optimal_route_{mode}.html"
        route_map.save(filename)
        print(f"\nМаршрут сохранен в файл {filename}")
    else:
        print("Не удалось визуализировать маршрут")
